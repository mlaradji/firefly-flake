...
current_version=$(nix eval -f ../flake.nix --raw inputs.data-importer-src.url | rg -o 'v?\d+\.\d+\.\d+')

if [[ "$current_version" == "$latest_version" ]]; then
    echo "data-importer: already at $current_version"
    exit 0
fi

echo "data-importer: $current_version -> $latest_version";
...
# Run composer2nix.
composer2nix --name "data-importer" \
    --composition=composition.nix
rm composer.json composer.lock

# Update the version number in flake.nix
sed -i "s:firefly-iii/data-importer/v\?\([0-9]\+\.\?\)\{3\}:firefly-iii/data-importer/${latest_version}:" ../flake.nix

nix fmt
# Check if the update worked by attempting a build.
nix build .#data-importer
nix flake check
exit $?
